(function() {
  ...
  test("new and parse", 3, function() {
    ...
    var collection = new PageableCollection(models, {parse: true});
  ...
  test("create with validate:true enforces validation", 3, function() {
    ...
    var ValidatingPageableCollection = Backbone.PageableCollection.extend({
      model: ValidatingModel
    });
    var col = new ValidatingPageableCollection();
  ...
  test("initialize", 1, function() {
    ...
    var coll = new PageableCollection;
    equal(coll.one, 1);
  });
  ...
  test("#1407 parse option on constructor parses collection and models", 2, function() {
    ...
    var c = new PageableCollection(model, {parse:true});
  ...
  test('merge without mutation', function () {
    ...
    var PageableCollection = Backbone.PageableCollection.extend({model: Model});
    var data = [{id: 1, child: {id: 2}}];
    var collection = new PageableCollection(data);
  ...
  test("`set` and model level `parse`", function() {
    ...
    var collection = new PageableCollection(model);
    collection.set({models: [
      {model: {id: 1}},
      {model: {id: 2}}
    ]}, {parse: true});
  ...
  test("#1894 - Push should not trigger a sort", 0, function() {
    ...
    new PageableCollection().push({id: 1});
  });
  ...
  test("`set` with non-normal id", function() {
    ...
    var collection = new PageableCollection({_id: 1});
  ...
  test("#1894 - `sort` can optionally be turned off", 0, function() {
    ...
    new PageableCollection().add({id: 1}, {sort: false});
  });
  ...
  test("#2606 - PageableCollection#create, success arguments", 1, function() {
    var collection = new Backbone.PageableCollection;
    collection.url = 'test';
    collection.create({}, {
  ...
  test("#2612 - nested `parse` works with `PageableCollection#set`", function() {
