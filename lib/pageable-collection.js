...
(function(root, factory) {
  ...
  var presenterMixin = Backbone.PageableCollectionPresenterMixin = {
    ...
    pager: function() {
      ...
      this.$el.html('<ul class="pager">'+this.getPrevLink() + content + this.getNextLink()+'</ul>'+ this.getPerPageSelector());
      return this;
    },
    slider: function() {
    ...
    changePerPage: function() {
      var e = document.getElementById("page-selector");
      var perPage = e.options[e.selectedIndex].value;
      window.location = this.setParam(window.location.toString(), 'perPage', perPage);
    },
    setParam: function(uri, paramKey, paramVal) {
    ...
    getPerPageSelector: function() {
      if (this.collection.state.showPerPageSelector) {
        var pageSelector = '<select class="page-selector" id="page-selector">',
          selectorValues = this.collection.state.pageSelectorValues,
          pages = [];
        for (var sv in selectorValues) {
          pages.push('<option value='+selectorValues[sv]+'>'+selectorValues[sv]+'</option>');
        }
  ...
  var PageableCollection = Backbone.PageableCollection = Backbone.Collection.extend({
    ...
    showPerPageSelector: true, // show page selector on right side
    pageSelectorValues : [60,120,180,240,300],
    ...
    constructor: function(models, options) {
      ...
      this.state.showPerPageSelector = options.showPerPageSelector || this.showPerPageSelector;
      this.state.pageSelectorValues = options.pageSelectorValues || this.pageSelectorValues;
      this.updateState();
      if (! this.presenter) {
    ...
    updateState: function() {
      ...
      // showPerPageSelector added to state
      this.state.showPerPageSelector = this.showPerPageSelector;
      this.state.pageSelectorValues = this.pageSelectorValues;
    },
    ...
    getPaginatorUrl: function(page, link) {
      ...
      if (self.appendParams) {
        queryString = search;
        if (search.length === 0) queryString += '?';
      } else {
        queryString += '?';
      }
      _.each(_.pairs(params), function(v, k) {
        ...
        param += v.join('=');
        if (self.appendParams) {
          var regex = new RegExp("((&)?" + v[0] + ")=(?:\\w+|\\d+)", 'gi');
      ...
      if (link) {
        return (_.result(this, 'linkUrl') || _.result(this, 'url') || '') + queryString;
      } else {
        return _.result(this, 'url', '') + queryString;
      }
    },
