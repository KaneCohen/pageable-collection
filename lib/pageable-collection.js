...
(function(root, factory) {
    ...
    getPerPageSelector: function() {
      ...
      if (c.showPerPageSelector) {
        var pageSelector = '<select class="page-selector">',
          ...
          activeValue = c.getParam(this.collection.params.perPage || 'perPage'),
          pages = [];
  ...
  var PageableCollection = Backbone.PageableCollection = Backbone.Collection.extend({
    ...
    pageSelectorValues : [10,30,60,120,180,240,300],
    ...
    getPaginatorUrl: function(page, link) {
      ...
      _.each(_.pairs(params), function(v, k) {
        ...
        if (self.appendParams) {
          var regex = new RegExp("([?&])(" + v[0] + ")=(?:\\w+|\\d+)", 'g');
          var matches = queryString.match(regex);
          if (matches.length) {
            var modifier = matches[0][0];
            queryString = queryString.replace(regex, modifier + param );
          }
        } else {
          if (k > 0) queryString += '&' + param;
        }
      });
