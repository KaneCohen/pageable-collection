...
(function(factory) {
	...
	var PageableCollection = Backbone.PageableCollection = Backbone.Collection.extend({
		...
		presenter:   null,
		linkUrl:     null,
		updateUrl:   true,     // Update url in address bar.
		...
		// Defaults.
		baseParams: {
			offset:      false,    // Send offset param to the server (based on page and per page).
			limit:       false,    // Send limit param.
		...
		constructor: function(models, options) {
			...
			this.state = _.extend({}, this.baseState, this.state, options.state);
			this.params = _.extend({}, this.baseParams, this.params, options.params);
    ...
    parse: function(resp, options) {
			var items = resp;
			if (_.isObject(resp) && _.has(resp, 'state')) {
				_.extend(this.state, resp.state);
				items = resp.items;
			}
			// Use data from header to update state.
			if (this.headerState) {
				this.state.perPage = options.xht.getResponseHeader('X-PerPage');
				this.state.currentPage = options.xht.getResponseHeader('X-CurrentPage');
				this.state.total = options.xht.getResponseHeader('X-Total');
			}
      return items;
    },
		...
		updateState: function() {
			...
			this.state.currentPage = ! isNaN(this.state.currentPage) ? this.state.currentPage : this.state.firstPage;
      ...
      this.items.reset(models, {silent: true});
		...
		fetchPage: function(page) {
			this.state.currentPage = page;
			if (this.mode == 'server') {
				if (this.cache && this.cachedPages[page] !== void 0) {
					this.reset(this.cachedPages[page]);
				} else {
					this.fetch({reset: true, url: this.getPaginatorUrl(page)});
				}
		...
		fetchFirstPage: function() {
			if (this.state.currentPage != this.state.firstPage) {
				return this.fetchPage(this.state.firstPage);
			}
		},
		...
		fetchLastPage: function() {
			if (this.state.lastPage > 0 && this.state.currentPage != this.state.lastPage) {
				return this.fetchPage(this.state.lastPage);
			}
		},
		...
		// Render pager-style links: <,5,6,7,>.
		pager: function() {
			return this.presenter.pager();
		},

		// Render slider-style links: Previous and next.
		slider: function() {
		...
		initPresenter: function() {
			...
			var View = Backbone.View.extend({
				...
				undelegateEvents: function() {
					...
					if (this.container) {
						this.container.off('scroll.delegateEvents' + this.cid);
					}
					return this;
				},
				render: function() {
				...
				pager: function() {
					var content = '';
					if (self.state.lastPage > 1) {
						if (self.state.lastPage < 13) {
							content = this.getPageRange(1, self.state.lastPage);
						} else {
				...
				openPage: function(e) {
					...
					if (self.updateUrl && self.mode != 'infinite') {
						window.history.pushState(null, null, self.getPaginatorUrl(page, true));
					}
					self.fetchPage(page);
					return false;
				},
				getPrevLink: function() {
					if (self.hasPrevPage()) {
						return '<li class="page-next"><a href="'+self.getPaginatorUrl(self.state.currentPage - 1, true)+'" data-page="'+(self.state.currentPage - 1)+'"><i class="icon iconSmallArrowLeft"></i></a></li>';
					}
					return '';
				},
				getNextLink: function() {
					if (self.hasNextPage()) {
						return '<li class="page-next"><a href="'+self.getPaginatorUrl(self.state.currentPage + 1, true)+'" data-page="'+(self.state.currentPage + 1)+'"><i class="icon iconSmallArrowRight"></i></a></li>';
					}
					return '';
				},
				getMoreLink: function() {
					if (self.hasNextPage()) {
						return '<li class="page-next page-more"><a href="'+self.getPaginatorUrl(self.state.currentPage + 1, true)+'" data-page="'+(self.state.currentPage + 1)+'">More</a></li>';
					}
					return '';
				},
				getPageRange: function(start, end) {
					...
					for (var page = start;page <= end; page++) {
						if (self.state.currentPage == page) {
				...
				getPagination: function() {
					...
					if (self.state.currentPage <= w) {
						ending = this.getFinish();
						return this.getPageRange(1, w + 2) + ending;
					} else if (self.state.currentPage >= self.state.lastPage - w) {
						start = self.state.lastPage - 8;
						content = this.getPageRange(start, self.state.lastPage);
				...
				getLink: function(page) {
					return '<a href="'+self.getPaginatorUrl(page, true)+'" data-page="'+page+'">'+page+'</a>';
				},
				getAdjacentRange: function() {
					start = self.state.currentPage - 3;

					return this.getPageRange(start, self.state.currentPage + 3);
				},
				getStart: function() {
					return this.getPageRange(1, 2)+this.getDots();
				},
				getFinish: function() {
					var content = this.getPageRange(self.state.lastPage - 1, self.state.lastPage);
